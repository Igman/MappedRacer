...
public class RaceController extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		case CREATE_RACE:
			...
			break;
		case ADD_RACER:
			addRacer(request, response);
			break;
		case ADD_ITEM:
			addItem(request, response);
			break;
		case END_RACE:
			endRace(request, response);
			break;
		case CHECK_IN:
			checkIn(request, response);
			break;
		case CHECK_RACE:
			checkRace(request, response);
			break;
		default:
	...
	private void createRace(HttpServletRequest request,
		...
		int raceID = race.createRace(name, endPoint, startTime, createDate,
				userID);
	...
	private boolean addItems(ArrayList<Item> items) {
	...
	/**
	 * This is where the race creator will add a new user to the race. It passes
	 * along the responsibility to the RacerController.
	 * 
	 * @param request
	 *            Request obj containing parameters passed and session returned.
	 * @param response
	...
	private void addRacer(HttpServletRequest request,
		...
		if (successful) {
			// Determines the next page.
			RequestDispatcher dispatcher = request
					.getRequestDispatcher("/WEB-INF/JSPs/race_created");
			dispatcher.forward(request, response);
		} else {
	 ...
	 * @throws IOException
	 */
	private void endRace(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
		int raceID = Integer.parseInt(request.getParameter("race_id"));

		// Sets the db to finished the race so players querying will
		boolean successful = race.finishRace(raceID);
		// TODO: what else happens when race finishes? we should calculate
		// scores here for the database.

		if (successful) {
			// Determines the next page.
			RequestDispatcher dispatcher = request
					.getRequestDispatcher("/WEB-INF/JSPs/race_results.jsp");
			dispatcher.forward(request, response);
		} else {
	...
	// TODO: Check in has to determine where it is and if it is at end or at
	// item.
	/**
	 * Check in from a user in a race that will then figure out if the check in
	 * is at the finish line or at a item. It also could include an image or
	 * comment.
	 * 
	 * @param request
	 *            Request obj containing parameters passed and session returned.
	...
	private void checkIn(HttpServletRequest request,
		...
		// TODO: Not sure the relationship between the two controllers will be
		// here.
		// Checkins jobs will be to determine if the user was in a item or at
		// the finish line.
		// Sets the db to finished the race so players querying will
		boolean successful = checkIn.userCheckIn(raceID, userID, picUrl,
				comment, location);

		if (successful) {
			// Determines the next page.
			RequestDispatcher dispatcher = request
					.getRequestDispatcher("/WEB-INF/JSPs/map.jsp");
			dispatcher.forward(request, response);
		} else {
	 ...
	 * @throws IOException
	 */
	private void checkRace(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
		...
		// Determines the next page.
		RequestDispatcher dispatcher = request
				.getRequestDispatcher("/WEB-INF/JSPs/race.jsp");
		dispatcher.forward(request, response);
	}

	/**
	 * This is how this controller will handle problems that occur with the DB
	 * it will send a message back to UI and tell it to go to the error page.
	 * 
	 * @param request
	 *            Request obj containing parameters passed and session returned.
	 * @param response
